#include "Render/Impl/RootSignatureImpl.h"
...
bool RootSignature_CreateImpl(RootSignature_t rs, const RootSignatureDesc& desc)
{
	...
	if (((uint32_t)desc.Flags & (uint32_t)RootSignatureFlags::AllowInputLayout) != 0)
	...
	for (uint32_t slotIt = 0; slotIt < desc.Slots.size(); slotIt++)
	{
		...
		break;
		case RootSignatureSlotType::DescriptorTable:
		{
	...
	for (uint32_t sampIt = 0; sampIt < desc.GlobalSamplers.size(); sampIt++)
	{
		...
		desc.Filter = Dx12_Filter(sd.comparison != SamplerComparisonFunc::None, sd.Min, sd.Mag, sd.Mip);
		desc.AddressU = Dx12_TextureAddressMode(sd.AddressU);
		desc.AddressV = Dx12_TextureAddressMode(sd.AddressV);
		desc.AddressW = Dx12_TextureAddressMode(sd.AddressW);
	...
	if ((desc.Flags & RootSignatureFlags::AllowInputLayout) != RootSignatureFlags::None)
	{
